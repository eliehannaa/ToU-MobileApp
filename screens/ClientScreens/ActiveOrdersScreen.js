...
const ActiveOrdersScreen = ({ navigation }) => {
  ...
  const handleProducts = async () => {
    try {
      ...
      const res = await axios.get(
        "/client/home/activeorders", //post request
        {
          headers: {
      ...
      const list = [];
      console.log("I am in ActiveOrdersScreen.js");
      console.log(res);
      ...
      for (let i = 0; i < res.data[0].aorders.length; i++) {
        list.push({
          id: pr[i].order._id,
          title: pr[i].product.title,
          price: pr[i].product.price,
          image: pr[i].product.image,
          url: pr[i].product.url,
          inStock: pr[i].product.inStock,
          status: pr[i].order.status,
          quantity: pr[i].order.quantity,
          cost: pr[i].order.cost.toFixed(2),
        });
      }
      console.log(list);
  ...
  const renderProduct = ({ item }) => (
    <ActiveOrderCard navigation={navigation} product={item} />
  );
