...
const ActiveOrderCard = ({ product }) => {
  ...
  const handleProofOfReceiptClicked = () => {
    ...
    if (status == "2") {
      ...
      Alert.alert(
        "Order Already Acquired",
        "You can only upload a proof of receipt once the order has been accepted."
      );
    } else if (status == "4") {
  ...
  const handleShippedClicked = () => {
    ...
    // if the status is 0, then the order has not been acquired yet
      ...
      Alert.alert(
        "Order Not Acquired",
        "You can only mark an order as shipped once it has been acquired."
      );
    }
    // if the status is 2, then the order has been shipped
      ...
      Alert.alert(
        "Order Already Shipped",
        "You can only mark an order as shipped once."
      );
    }
  ...
  const handleArrivedClicked = () => {
    if (status == "4") {
      ...
      Alert.alert(
        "Order Not Acquired",
        "You can only mark an order as arrived once it has been acquired."
      );
    } else if (status == "3") {
      Alert.alert(
        "Order Not Shipped",
        "You can only mark an order as arrived once it has been shipped."
      );
    } else if (status == "5") {
      // This means the order has already arrived to Lebanon
      Alert.alert(
        "Order Already Arrived",
        "You can only mark an order as arrived once."
      );
    } else if (status == "6") {
  ...
  const handleOrderArrived = () => {
    ...
    Alert.alert(
      "Order Arrived",
      "Are you sure you want to mark the order as arrived to Lebanon?",
      [
        {
          text: "Cancel",
          ...
          onPress: async () => {
            ...
            try {
              ...
              const res = await axios.post(
                "/traveler/home/activeorders/" + product.id + "/markarrived",
                {},
                {
                  headers: {
  ...
  const handleOrderShipped = () => {
    ...
    Alert.alert(
      "Order Shipped",
      "Are you sure you want to mark the order as shipped?",
      [
        {
          text: "Cancel",
          onPress: () => console.log("Cancel Pressed"),
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            try {
              const res = await axios.post(
                "/traveler/home/activeorders/" + product.id + "/markassent",
                {},
                {
                  headers: {
                    "Content-Type": "application/json",
  ...
  const submitProofOfReceipt = async () => {
    // helper function to upload the proof of receipt to the server
    try {
      console.log("submitProofOfReceipt");
      const token = await AsyncStorage.getItem("AccessToken");
      ...
      // });
      const res = await axios.post(
        "/traveler/home/activeorders/" + product.id + "/uploadproof",
        {
          file: {
            uri: "proofOfReceiptUri",
            name: "proofOfReceiptName",
  ...
  const renderTimelineStage = (stage) => {
    ...
    return (
      <View style={styles.timelineStageContainer}>
        <View style={[styles.timelineStage, { backgroundColor: stageColor }]} />
